...
const certificate = document.getElementById('certificate'); // Контейнер с изображением диплома

// Функция для отображения результата
function displayResult(correct, total) {
    if (correct === total) {
        // Если все ответы правильные
        resultText.textContent = `Поздравляем! Вы окончили колледж с дипломом о профессиональном образовании.`; // Сообщение об успешном завершении
        resultText.style.color = 'green'; // Установка зелёного цвета текста
        ...
        certificate.style.display = 'block'; // Показываем изображение диплома
        localStorage.setItem('stage2Completed', 'true'); // Сохраняем флаг завершения этапа в localStorage
    } else {
        ...
        resultText.style.color = 'red'; // Установка красного цвета текста
        retryBtn.style.display = 'inline-block'; // Показываем кнопку "Пройти ещё раз"
        nextBtn.textContent = 'Пройти обучение';
        nextBtn.style.display = 'inline-block';
        certificate.style.display = 'none'; // Скрываем изображение диплома
    }
}
...
function addEventListeners() {
    retryBtn.addEventListener('click', () => {
        ...
        window.location.href = 'questions2.html?stage=2'; // Перезапуск экзамена этапа 2
    });
    ...
    nextBtn.addEventListener('click', () => {
        ...
        window.location.href = 'questions2.html?stage=2'; // Переход в обучение текущего этапа
    });
}
